...
const COLORS = {
  ...
  redLight: '#FDF0F0', accentLight: '#FCF3ED', overlay: 'rgba(64, 64, 64, 0.5)',
};
 ...
 * PostCard del feed.
 *
 * IMAGEN NO EXPANDE — al tocar la imagen, el evento va al onPress de la
 * TouchableOpacity exterior (que navega al PostDetail). Esto se logra con
 * ExpandableImage expandable={false} + onPress={onPress}.
 *
 * En PostDetailScreen, en cambio, ExpandableImage usa el modo default
 * (expandable=true) para abrir el modal fullscreen al tocar.
 */
function PostCard({ post, onPress, onPressAuthor, onReact, isModerador, onShowMenu }: any) {
...
export default function SocialScreen({ navigation }: any) {
  const cachedFeed = communityCache.peek<any>(CK.socialFeed);
  ...
  const [menuModal, setMenuModal] = useState({ visible: false, postId: '', comunidadId: '' });
  const [deleteModal, setDeleteModal] = useState({ visible: false, postId: '', comunidadId: '' });
  const [actionModal, setActionModal] = useState<any>({ visible: false, autor: null, comunidadId: '' });
  const [errorModal, setErrorModal] = useState({ visible: false, message: '' });

  const fetchData = useCallback(async (force = false) => {
    if (!force) {
      const fresh = communityCache.get<any>(CK.socialFeed, TTL.socialFeed);
      if (fresh) {
        setCommunities(fresh.communities); setAllPosts(fresh.allPosts);
        setDailyForum(fresh.dailyForum); setForumCommunities(fresh.forumCommunities);
    ...
    try {
      ...
      const flatPosts = postsResults.flat().sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
      ...
      communityCache.set(CK.socialFeed, {
        communities: comms, allPosts: flatPosts,
        dailyForum: dailyForumData.foro || null, forumCommunities: dailyForumData.comunidades || [],
      });
    } catch (err) {
  ...
  const confirmDeletePost = async () => {
    ...
    try {
      ...
      if (post && !post.es_mio && isModerador) setActionModal({ visible: true, autor: post.autor, comunidadId });
    } catch (err: any) {
    ...
    <View style={styles.container}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          <TouchableOpacity style={[styles.iconBtn, !hasCommunities && styles.iconBtnDisabled]}
            onPress={() => hasCommunities && navigation.navigate('CreatePost', { communities })} disabled={!hasCommunities}>
            <Feather name="plus" size={22} color={hasCommunities ? COLORS.text : COLORS.lightMuted} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.iconBtn} onPress={() => navigation.navigate('MyCommunities', { communities })}>
            <Feather name="users" size={20} color={COLORS.text} />
          </TouchableOpacity>
          <TouchableOpacity style={styles.profileBtn} onPress={() => navigation.navigate('SocialProfile', { isOwn: true })}>
      ...
      >
        <TouchableOpacity style={styles.forumCard} onPress={() => navigation.navigate('DailyForum', { communities: forumCommunities })} activeOpacity={0.8}>
        ...
        <View style={styles.sectionHeader}>
          ...
          {allPosts.length > 0 && <View style={styles.badge}><Text style={styles.badgeText}>{allPosts.length}</Text></View>}
        </View>

        {!hasCommunities ? (
          <View style={styles.emptyContainer}>
            <View style={styles.emptyIcon}><Feather name="users" size={32} color={COLORS.accent} /></View>
            <Text style={styles.emptyTitle}>Sin comunidades</Text>
            <Text style={styles.emptyText}>Unete a una comunidad para ver publicaciones.</Text>
          ...
          <View style={styles.emptyContainer}>
            ...
            <Text style={styles.emptyText}>Se el primero en compartir algo.</Text>
          ...
          allPosts.map((post) => (
            <PostCard
              ...
              onPress={() => navigation.navigate('PostDetail', { post, communityId: post.comunidad_id, community: communities.find((c: any) => c.id === post.comunidad_id) })}
      ...
      <CustomModal visible={actionModal.visible} title="Tomar acciones"
        ...
        buttons={[
          ...
          { text: 'Si', onPress: () => { setModTarget(actionModal.autor); setModCommunityId(actionModal.comunidadId); setModModalVisible(true); } },
        ]}
      ...
      <ModerationActionsModal visible={modModalVisible} communityId={modCommunityId} targetUser={modTarget}
