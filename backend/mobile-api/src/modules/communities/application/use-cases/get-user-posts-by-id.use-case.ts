...
export class GetUserPostsByIdUseCase {
  ...
  async execute(robleId: string, requestorUuid: string) {
    ...
    // Obtener data del usuario objetivo (avatar, nombre)
    const targetUser = await this.dbService.findById('usuarios', robleId, masterToken);
    ...
    // Solo posts de comunidades en comun
    const posts = allPosts.filter(
      (p: any) => !p.eliminado && myCommunityIds.has(p.comunidad_id)
    );

    // Enriquecer con autor (avatar) + imagen del post
