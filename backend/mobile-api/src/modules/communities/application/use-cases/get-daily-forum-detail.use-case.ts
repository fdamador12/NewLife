...
export class GetDailyForumDetailUseCase {
  ...
  async execute(foroId: string, comunidadId: string, usuarioUuid: string) {
    ...
    // Enriquecer respuestas con autor (con avatar), likes y comentarios
    const enrichedReplies = await Promise.all(
      replies.map(async (reply: any) => {
        ...
        // findById puede devolver objeto directo o {rows: [...]}
        const autor    = (autorRes && (autorRes as any).rows) ? (autorRes as any).rows[0] : autorRes;
        ...
        // Enriquecer comentarios con autor (con avatar)
        const enrichedComments = await Promise.all(
          activeComments.map(async (c: any) => {
            ...
            return {
              ...
              autor: {
                id: c.autor_id,
                nombre: cAutor?.nombre || 'Usuario',
                avatar_url: cAutor?.avatar_url || null,
              },
            };
          })
        );
